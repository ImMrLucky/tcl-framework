 ...
 */
export function loadPlanConfig(): PlansConfig {
  try {
    const configPath = join(__dirname, 'plans.json');
    const configContent = readFileSync(configPath, 'utf-8');
    const config = JSON.parse(configContent) as PlansConfig;
 ...
 */
export function validatePlanConfig(config: any): asserts config is PlansConfig {
  ...
  const requiredTiers: PlanTier[] = ['SANDBOX', 'TEAM', 'ENTERPRISE'];
  for (const tier of requiredTiers) {
    ...
    const validCapabilities = Object.values(Capability) as string[];
  ...
  const invalidTiers = tierKeys.filter(tier => !requiredTiers.includes(tier as PlanTier));
 ...
 */
export function getCapabilitiesForTier(tier: PlanTier): Capability[] {
  const config = getPlanConfig(tier);
  return config.capabilities as Capability[];
}
