...
import type { ValidateInput, ValidateOutput, TrajectoryReport, TrajectorySegment, DestructiveClaim } from "./types.js";
import type { LLMAdapter } from "./adapters/llm_adapter.js";

// This will be imported from orchestrator (avoid circular dependency)
...
export async function computeTrajectory(
  ...
  const numSegments = Math.min(
    Math.ceil(turns.length / windowTurns),
    maxSegments
  );
  ...
  for (let segIdx = 0; segIdx < numSegments; segIdx++) {
    ...
    if (segmentTurns.length === 0) break;
    ...
    const segmentInput: ValidateInput = {
    ...
    try {
      ...
      const segment: TrajectorySegment = {
        ...
        spectral: segmentResult.report.spectral ? {
          ...
          fingerprint: (segmentResult.report.spectral as any).fingerprint
        } : undefined,
  ...
  // Compute summary
  const overallScores = segments.map(s => s.scores.overall).filter(s => s !== null && s !== undefined) as number[];
  
  let worstSegmentIndex: number | null = null;
  let worstOverallScore: number | null = null;
