...
export function sanitizeTranscriptForScoring(input: string): TranscriptSanitizerResult {
  let normalizedInlineSpeakerBoundaries = 0;
  const withBoundaries = input.replace(INLINE_SPEAKER_BOUNDARY, (_match, speaker: string) => {
  ...
  if (removedAnnotationLines > 0) diagnostics.push(`Removed ${removedAnnotationLines} annotation/non-transcript lines`);
  if (normalizedInlineSpeakerBoundaries > 0) diagnostics.push(`Normalized ${normalizedInlineSpeakerBoundaries} inline speaker boundaries`);
  if (unknownSpeakerLines > 0) diagnostics.push(`Dropped ${unknownSpeakerLines} unknown speaker/non-turn lines`);
...
export function isContaminatedClaimText(text: string): boolean {
  return CONTAMINATION_PATTERNS.some(pattern => pattern.test(text));
}

export function countSpeakerLabelsInClaim(text: string): number {
