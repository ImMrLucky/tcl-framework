import { Claim, Source, Violation } from "./types";

function normalize(s: string) {
  return s.toLowerCase().replace(/[^a-z0-9\s]/g, "").replace(/\s+/g, " ").trim();
}

function tokenOverlapScore(claim: string, source: string): number {
 ...
 */
export function attachEvidenceAndFindViolations(
  claims: Claim[],
  ...
  const missing: { claimId: string; reason: string }[] = [];
  ...
  for (const claim of claims) {
    ...
    for (const src of sources) {
      ...
      if (!best || sc > best.score) best = { sid: src.id, score: sc };
    }
