...
import { supabaseAdmin, generateApiKey, hashApiKey, logAudit } from '../supabase.js';
...
export function setupApiKeyRoutes(app: express.Application) {
  // ============================================================================
  ...
  app.get('/api/api-keys', async (req, res) => {
    try {
      ...
      const response: ApiKeyResponse[] = (keys || []).map((k: any) => ({
        id: k.id,
        name: k.name,
        prefix: k.prefix,
        mode: (k.mode || 'SANDBOX') as 'SANDBOX' | 'PROD',
  ...
  app.post('/api/api-keys', async (req, res) => {
    try {
      ...
      const keyMode = (mode || 'SANDBOX') as 'SANDBOX' | 'PROD';
