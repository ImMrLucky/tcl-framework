 ...
 */
function canonicalizeText(text: string | undefined | null): string {
  if (!text) return '';
  return text
    .trim()
...
export function generateDeterministicIssueId(params: {
  ...
  // Fallback: if canonical fields not available, use these
  issueKey?: string;
 ...
 */
export function generateIssueIdFromIssue(
  issue: IssueV2,
  orgId: string,
  evaluationId: string
): string {
