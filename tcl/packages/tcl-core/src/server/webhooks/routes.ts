...
export function setupWebhookRoutes(app: express.Application) {
  ...
  app.get('/api/webhooks', async (req, res) => {
    try {
      ...
      const response: WebhookEndpoint[] = (webhooks || []).map((w: any) => ({
        id: w.id,
        orgId: w.org_id,
        url: w.url,
        enabled: w.enabled,
        mode: (w.mode || 'SANDBOX') as 'SANDBOX' | 'PROD',
  ...
  app.post('/api/webhooks', async (req, res) => {
    try {
      ...
      const webhookMode = (mode || 'SANDBOX') as 'SANDBOX' | 'PROD';
      ...
      const validEvents = Array.isArray(events) && events.length > 0 
        ? events.filter((e: any) => typeof e === 'string')
        : ['analysis.completed'];
      ...
      const response: WebhookEndpoint = {
        id: data.id,
        orgId: context.orgId,
        url: data.url,
        enabled: data.enabled,
        mode: (data.mode || 'SANDBOX') as 'SANDBOX' | 'PROD',
  ...
  app.post('/api/webhooks/:id/test', async (req, res) => {
    try {
      ...
      const testPayload: WebhookTestPayload = {
        event: 'webhook.test',
        mode: (webhook.mode || 'SANDBOX') as 'SANDBOX' | 'PROD',
        orgId: context.orgId,
            ...
            if (response.ok) {
              await supabaseAdmin
                ...
                .update({
                  ...
                  delivery_count: (webhook as any).delivery_count + 1 || 1,
                })
                .eq('id', id);
            } else {
              await supabaseAdmin
