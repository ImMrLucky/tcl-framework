...
export class JSONTurnsNormalizer implements Normalizer {
  name = "json-turns";
  extensions = [".json"];
  ...
  async normalize(content: Buffer | string, options: NormalizerOptions): Promise<NormalizerResult> {
    ...
    for (let i = 0; i < rawTurns.length; i++) {
      ...
      if (!turnText || typeof turnText !== "string") continue;
  ...
  private extractTurns(data: any): { turns: any[]; format: string } {
    ...
    if (data.Transcript?.Segments && Array.isArray(data.Transcript.Segments)) {
      // Amazon Connect format
      return { 
        turns: data.Transcript.Segments.map((s: any) => ({
          text: s.Content,
  ...
  private extractConversationMeta(data: any): NormalizedConversation["conversation"] {
    const meta: NormalizedConversation["conversation"] = {};
  ...
  private detectChannel(data: any): NormalizedConversation["channel"] {
