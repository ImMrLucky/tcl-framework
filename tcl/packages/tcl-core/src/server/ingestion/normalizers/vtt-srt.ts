...
export class VTTSRTNormalizer implements Normalizer {
  ...
  private parseCues(text: string, isVTT: boolean): Cue[] {
    const cues: Cue[] = [];
  ...
  private mergeConsecutiveTurns(turns: Turn[], participantsMap: Map<string, Participant>): Turn[] {
  ...
  private createEmptyConversation(content: Buffer | string, format: string): NormalizedConversation {
    return {
      schemaVersion: NORMALIZED_SCHEMA_VERSION,
      channel: "call",
