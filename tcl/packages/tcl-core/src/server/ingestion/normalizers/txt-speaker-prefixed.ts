...
export class TxtSpeakerPrefixedNormalizer implements Normalizer {
  ...
  async normalize(content: Buffer | string, options: NormalizerOptions): Promise<NormalizerResult> {
    ...
    const warnings: string[] = [...sanitized.diagnostics];
    ...
    const singleUnknownSpeakerBlob =
      turns.length === 1 &&
      ...
      if (nt.length >= 2 && nt.length > turns.length) {
        ...
        for (const t of nt) {
          const turn = this.createTurn(
            ...
            participantsMap,
            options,
            charAcc + t.text.length
          );
          turns.push(turn);
          charAcc += t.text.length + 1;
        }
        warnings.push(
          `Parsed ${turns.length} turns via bracket-timestamp / speaker-line engine (normalizeTranscript)`
        );
      }
    }
