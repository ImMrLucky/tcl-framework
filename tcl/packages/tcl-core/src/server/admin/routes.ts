...
import { PlanTier, PlanStatus } from '../plans/capabilities.js';
...
export function setupAdminRoutes(app: express.Application) {
  ...
  app.post('/api/admin/switch-org', requireSuperuser, async (req, res) => {
    try {
      ...
      await logAdminAction(
        ...
        'admin.switch_org',
        orgId,
        { orgName: org.name, planTier: org.plan_tier }
      );
  ...
  app.post('/api/admin/internal-org/plan', requireSuperuser, async (req, res) => {
    try {
      ...
      await logAdminAction(
        adminContext.userId,
        'admin.internal_org.plan_set',
        orgId,
        { planTier, planStatus, billingMode, orgName: org.name }
      );
