 ...
 */
function parseCsvEnv(envVar: string | undefined): string[] {
  if (!envVar) return [];
  return envVar
    .split(',')
 ...
 */
export async function maybeGrantSuperuser(userId: string, email: string): Promise<boolean> {
  if (!supabaseAdmin) {
