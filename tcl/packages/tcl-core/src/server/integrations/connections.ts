...
export interface IntegrationConnection {
  ...
  config: Record<string, any>; // Opaque config JSON
  errorMessage?: string;
  createdAt: string;
  updatedAt: string;
  lastSyncAt?: string;
  comingSoon?: boolean; // True if connector not yet implemented
}
...
export function setupIntegrationConnectionsRoutes(app: express.Application) {
  // ============================================================================
  ...
  app.get('/api/integrations', async (req, res) => {
    try {
      ...
      const connectionMap = new Map<IntegrationType, IntegrationConnection>();
    ...
    try {
      ...
      if (existing) {
        ...
        connection = {
          ...
          type: created.type as IntegrationType,
          status: created.status as IntegrationStatus,
