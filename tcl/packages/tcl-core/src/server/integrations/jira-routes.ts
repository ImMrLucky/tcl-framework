 ...
 */
export function setupJiraRoutes(app: express.Application) {
  // ============================================================================
  ...
  // ============================================================================
  app.post(
    '/api/integrations/jira/tickets/from-issue',
    requireEntitlement('integrations'),
    async (req, res) => {
      try {
        const context = await getOrgContext(req);
        ...
        if (evaluationId) {
          ...
          if (evaluation) {
            ...
            issue = allIssues.find((i: any) => i.issueId === issueId);
          }
        }
        ...
        const evaluationLink = evaluationId
          ? `${(req as any).headers.host || 'protectqa.com'}/evaluations/${evaluationId}`
          : undefined;
  ...
  // ============================================================================
  app.post(
    '/api/integrations/jira/tickets/from-issues',
    requireEntitlement('integrations'),
    async (req, res) => {
      try {
        const context = await getOrgContext(req);
        ...
        if (evaluationId) {
          ...
          if (evaluation) {
            const report = evaluation.report as any;
            const allIssues = report?.allIssuesV2 || report?.topIssuesV2 || [];
            issues = allIssues.filter((i: any) => issueIds.includes(i.issueId));
          }
        }
  ...
  // ============================================================================
  app.post(
    '/api/integrations/jira/tickets/from-case',
    requireEntitlement('integrations'),
    async (req, res) => {
      try {
        const context = await getOrgContext(req);
