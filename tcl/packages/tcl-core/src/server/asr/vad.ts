 ...
 */
export async function prepareWavForAsr(inputPath: string): Promise<{ wavPath: string; durationMs: number }> {
  ...
  return new Promise((resolve, reject) => {
    ...
    probeProcess.on('close', (code) => {
      ...
      normalizeProcess.stderr.on('data', (data: Buffer) => {
 ...
 */
export async function applyVad(wavPath: string): Promise<VadResult> {
  ...
  if (VAD_MODE === 'silenceremove') {
    try {
      ...
      getDurationProcess.stderr.on('data', (data: Buffer) => {
