...
function dedupePacks(packs: DomainPack[]): DomainPack[] {
...
function isApplicable(claim: Claim, pack: DomainPack): boolean {
  const role = claim.meta?.speakerType ?? "unknown";
  const allowed = pack.appliesToRoles ?? ["agent", "supervisor"];
  return (allowed as readonly string[]).includes(role);
}

function ruleMatchesClaim(rule: DomainRule, claim: Claim): boolean {
  const st = claim.meta?.speakerType ?? "unknown";
  if (rule.appliesToSpeakerTypes?.length) {
    if (!rule.appliesToSpeakerTypes.includes(st as never)) return false;
  }
  return rule.patterns.some(p => p.test(claim.text));
}

function narrativeFor(rule: { summary: string; detail: string }, claim: Claim): Pick<IssueV2["what"], "plainEnglishSummary" | "whyItMatters"> {
...
function defaultMissingEvidenceForType(type: string): string[] {
  if (/PROTECTQA|GUARANTEE|APPROVAL|QUALIF/i.test(type)) return ["carrier_underwriting_rule", "application_status", "product_availability"];
  if (/PRIVACY/i.test(type)) return ["privacy_policy_excerpt"];
  if (/LICENSE/i.test(type)) return ["agent_license_record", "state_jurisdiction"];
  if (/RATE|PRIC/i.test(type)) return ["carrier_rate_table", "underwriting_quote"];
  if (/AI_TOOL|TOOL_USE/i.test(type)) return ["tool_result", "api_response"];
  return ["approved_source", "policy_or_kb_citation"];
}

function recommendedActionLabel(type: string, severity: string): string {
  if (/AI_/i.test(type)) return severity === "critical" ? "AI policy update & compliance review" : "Review AI prompts & grounding";
  if (/PROTECTQA|GUARANTEE|DISCLOSURE|COMPLIANCE|HUMAN_/i.test(type)) return "Compliance review & ProtectQA rule check";
  if (/DRIFT/i.test(type)) return "Review approved script alignment";
  return "Conversation risk review";
}

function businessImpactFor(type: string): string {
  if (/GUARANT|PAYOUT|RATE|FINANCE/i.test(type)) return "Regulatory / customer-dispute risk";
  if (/PRIVACY|LICENSE/i.test(type)) return "Regulatory risk";
  if (/AI_|HALLUCIN/i.test(type)) return "AI reliability issue";
  if (/CHURN|OBJECTION|CONFUSION/i.test(type)) return "Customer confusion / churn signal";
  return "Compliance or accuracy risk";
}

function makeIssue(
  pack: DomainPack,
  rule: { type: string; severity: "low" | "medium" | "high" | "critical"; summary: string; detail: string; saferVersion: string; tags: string[]; requiredEvidence?: string[] },
  claim: Claim,
  context: RunContext
): IssueV2 {
  ...
  return {
    ...
    type: rule.type as IssueV2["type"],
    ...
    who: {
      speaker:
        claim.meta?.speakerType === "customer"
          ? "CUSTOMER"
...
function runRules(pack: DomainPack, claims: Claim[], context: RunContext): IssueV2[] {
  const issues: IssueV2[] = [];
  for (const claim of claims) {
    if (!isApplicable(claim, pack)) continue;
    for (const rule of pack.rules) {
      if (!ruleMatchesClaim(rule, claim)) continue;
      issues.push(makeIssue(pack, rule, claim, context));
    }
    for (const phrase of pack.forbiddenPhrases) {
      if (phrase.pattern.test(claim.text)) issues.push(makeIssue(pack, phrase, claim, context));
    }
  }
  return issues;
}

function runRequiredDisclosures(pack: DomainPack, claims: Claim[], context: RunContext): IssueV2[] {
  const issues: IssueV2[] = [];
  const applicable = claims.filter(c => isApplicable(c, pack));
  if (applicable.length === 0) return issues;
  const allText = applicable.map(c => c.text).join(" \n ");

  for (const disclosure of pack.requiredDisclosures) {
    if (!disclosure.trigger.test(allText)) continue;
    if (disclosure.disclosure.test(allText)) continue;
...
export function runDomainPack(pack: DomainPack, claims: Claim[], context: RunContext): DomainPackRunResult {
  const issues = [...runRules(pack, claims, context), ...runRequiredDisclosures(pack, claims, context)];
  return { packId: pack.id, issues };
}

export function runDomainPacks(packs: DomainPack[], claims: Claim[], context: RunContext): IssueV2[] {
