 ...
 */
export function computeEnhancedScores(input: ScoreComputationInput): EnhancedScores {
  ...
  if (hasExternalSources) {
    const verifiedClaims = claims.filter(c =>
      c.verification && c.verification.status === "verified"
    );
  ...
  const directContradictions = contradictions.filter(c =>
    c.contradictionType === "direct" || !c.contradictionType
  );
  ...
  const consistencyScore = Math.max(
    0, 
    Math.round(100 - directContradictions.length * contradictionPenalty)
  );
 ...
 */
export function computeSummaryStats(input: ScoreComputationInput): SummaryStats {
  ...
  // Count needs review
  const needsReviewCount = contradictions.filter(c =>
    c.contradictionType === "needs_review" || c.contradictionType === "topic_mismatch"
  ).length;
