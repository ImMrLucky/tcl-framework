...
export type FactualTruthClassification =
  | "TRUE_SUPPORTED"
  | "TRUE_BUT_NEEDS_DISCLOSURE"
  | "MISLEADING"
  | "LIKELY_FALSE"
  | "FALSE_BY_RULE"
  | "UNVERIFIABLE";
...
export interface FactualTruthResult {
  ...
  classifications: Array<{ claimId: string; classification: FactualTruthClassification; penalty: number; reasons: string[] }>;
}
...
export function evaluateFactualTruth(
  claims: Claim[],
  issues: IssueV2[],
  context: { hasExternalEvidence: boolean }
): FactualTruthResult {
  const classifications: FactualTruthResult["classifications"] = [];
  let penalty = 0;

  for (const claim of claims.filter(isAgentClaim)) {
    const reasons: string[] = [];
    let classification: FactualTruthClassification = "UNVERIFIABLE";
