...
export function buildDashboardSummary(input: {
  ...
  topUnsupported: Array<{ claimText: string; missing: string[] }>;
  nextActions: string[];
}): DashboardSummary {
  ...
  const topDrift =
    input.drift?.driftTimeline?.length && input.drift.driftIssues.length
...
function sevRank(s: SeverityV2): number {
  return s === "critical" ? 4 : s === "high" ? 3 : s === "medium" ? 2 : 1;
}

function summarizeSpeakers(claims: Claim[]): string {
...
function inferTopics(text: string): string {
  ...
  const topics: string[] = [];
  if (/\b(final expense|burial|death benefit)\b/.test(t)) topics.push("final expense");
  if (/\b(refund|support|ticket)\b/.test(t)) topics.push("support");
  if (/\b(soc|hipaa|integration)\b/.test(t)) topics.push("saas/security");
