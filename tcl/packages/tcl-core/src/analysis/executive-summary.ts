...
export interface ExecutiveSummary {
  ...
  highlights: string[];
  recommendedActions: Array<{ kind: "COACHING" | "COMPLIANCE" | "PROCESS" | "LEGAL"; action: string; priority: "high" | "medium" | "low" }>;
...
function gradeFromOverall(overall: number, criticalCount: number): TrustGrade {
  if (criticalCount > 0 && overall <= 35) return "F";
...
function severityRank(s: IssueV2["severity"]): number {
  return s === "critical" ? 4 : s === "high" ? 3 : s === "medium" ? 2 : 1;
}

function categoryFromIssue(issue: IssueV2): string {
  if (issue.compliance?.tags?.includes("hallucination_risk")) return "Hallucination";
  if (issue.compliance?.tags?.includes("drift")) return "Drift";
  if (issue.compliance?.tags?.includes("cross_turn")) return "Cross-turn";
  if (issue.type === "MISSING_REQUIRED_DISCLOSURE") return "Disclosure";
  if (issue.type === "SPEAKER_ATTRIBUTION_FAILURE") return "Attribution";
  if (issue.category === "compliance") return "Compliance";
  if (issue.category === "billing") return "Billing";
  if (issue.category === "consistency") return "Consistency";
  return "Other";
}

function quoteForIssue(issue: IssueV2): string {
  return issue.what?.claimText
    ?? issue.evidence?.refs?.[0]?.quote
    ?? "";
}

function actionsFromIssues(issues: IssueV2[]): ExecutiveSummary["recommendedActions"] {
  const actions: ExecutiveSummary["recommendedActions"] = [];
  const types = new Set(issues.map(i => i.type));
  if (
    types.has("GUARANTEED_APPROVAL") ||
    ...
    actions.push({
      kind: "COMPLIANCE",
      action:
        "Run ProtectQA/compliance review: use carrier-dependent qualification language—not guaranteed approval—in line with underwriting rules.",
      priority: "high",
    });
  }
  ...
  if (types.has("MISSING_REQUIRED_DISCLOSURE") || [...types].some(t => /^PROTECTQA_MISSING_/i.test(t))) {
    actions.push({
      kind: "PROCESS",
      action:
        "Add required disclosures (carrier approval dependency, underwriting, waiting periods) to workflows and AI prompts.",
      priority: "medium",
    });
  }
  ...
  if (
    types.has("HALLUCINATED_AUTHORITY") ||
  ...
  if (types.has("COMMITMENT_ESCALATION_DRIFT") || types.has("PROTECTQA_AI_QUALIFICATION_DRIFT")) {
    actions.push({
      kind: "COMPLIANCE",
      action:
        "Calibrate drift: qualify early estimates and forbid escalation to guaranteed approvals without underwriting evidence.",
      priority: "medium",
    });
  }
...
function highlightsFromScores(input: ExecutiveSummaryInput): string[] {
  const out: string[] = [];
  if (input.risk.criticalCount === 0 && input.risk.highCount === 0) {
    out.push("No critical or high-severity compliance issues detected.");
  }
  if (input.scores.compliance >= 85) out.push(`Compliance score ${input.scores.compliance} indicates the agent stayed within safe sales language.`);
  if (input.scores.factualTruth >= 80) out.push(`Factual truth score ${input.scores.factualTruth} indicates claims were appropriately qualified.`);
  if (input.scores.drift >= 90) out.push(`Drift score ${input.scores.drift} indicates consistent commitment language across the call.`);
  return out;
}
...
export function computeExecutiveSummary(input: {
  ...
  const issues = (input.aggregatedIssues ?? []) as any[];
  const flatIssues: IssueV2[] = issues.flatMap((agg: any) => agg?.issues || agg?.allIssues || (agg?.issueKey ? [agg] : []));
...
export function buildExecutiveSummary(input: ExecutiveSummaryInput): ExecutiveSummary {
  ...
  const headline =
    trustGrade === "A" ? "Call meets compliance and truthfulness expectations." :
  ...
  const oneLineVerdict =
    input.risk.criticalCount > 0 ? `${input.risk.criticalCount} critical and ${input.risk.highCount} high-severity issues detected; do not release without review.` :
    ...
    scoreBreakdown: [
      {
        ...
        description:
          "Primary Conversation Truth & Risk score — blends compliance, truthful/supported claims, disclosures, drift, hallucination indicators, evidence, and speaker confidence.",
      },
