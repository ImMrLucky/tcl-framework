...
import type { Claim, IssueV2, IssueTypeV2 } from "../types.js";
...
export interface ConsistencyResult {
  ...
  pairs: Array<{ earlier: ConsistencyEvent; later: ConsistencyEvent; reason: string }>;
}

const HEALTH_KEYWORDS: Array<{ key: string; pattern: RegExp }> = [
...
function extractEntities(text: string): InternalEntity[] {
  const out: InternalEntity[] = [];

  const healthKeys = HEALTH_KEYWORDS.filter(h => h.pattern.test(text)).map(h => h.key);
  if (healthKeys.length > 0) out.push({ topic: "health", keys: healthKeys, numbers: [] });
  ...
  if (numbers.length > 0) out.push({ topic: "amount", keys: numbers.map(n => `$${n}`), numbers });

  if (APPROVAL_AFFIRM.test(text)) out.push({ topic: "approval", keys: ["approval_affirmed"], numbers: [] });
  else if (APPROVAL_QUALIFY.test(text)) out.push({ topic: "approval", keys: ["approval_qualified"], numbers: [] });

  return out;
}

function speakerType(claim: Claim): ConsistencyEvent["kind"] | null {
  const role = claim.meta?.speakerType;
  if (role === "customer") return "customer_fact";
  if (role === "agent" || role === "supervisor") {
    if (HEALTH_DISMISSAL.test(claim.text)) return "agent_dismissal";
    if (NUMERIC_QUOTE.test(claim.text)) return "numeric";
    if (APPROVAL_AFFIRM.test(claim.text)) return "commitment";
    return "agent_assertion";
  }
  return null;
}

function eventsForClaim(claim: Claim): ConsistencyEvent[] {
...
export function runCrossTurnConsistency(
  claims: Claim[],
  context: { runId: string; conversationId: string; evidenceMode: "TRANSCRIPT_ONLY" | "TRANSCRIPT_PLUS_EXTERNAL" }
): ConsistencyResult {
  const events = claims
  ...
  const pairs: ConsistencyResult["pairs"] = [];
