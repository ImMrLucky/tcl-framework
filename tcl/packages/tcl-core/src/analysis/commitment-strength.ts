...
export function scoreCommitmentStrength(claim: Claim): CommitmentScore {
  const text = claim.text || "";
  const speakerType = (claim.meta?.speakerType as CommitmentScore["speakerType"]) ?? "unknown";
...
export function scoreCommitmentStrengthMany(claims: Claim[]): CommitmentScore[] {
...
export function topicKey(text: string): string {
  ...
  if (/\b(approval|approved|qualify|eligibility|denial)\b/.test(lower)) return "approval";
  if (/\b(carrier|underwriting)\b/.test(lower)) return "carrier";
  if (/\b(death benefit|payout|family will|beneficiary)\b/.test(lower)) return "payout";
  if (/\b(coverage|policy|premium|rate|graded|modified|waiting period)\b/.test(lower)) return "policy_terms";
  if (/\b(diabetes|cancer|heart|oxygen|hospitalization|medical|prescription|health)\b/.test(lower)) return "health";
  if (/\b(license|state|privacy|data sharing)\b/.test(lower)) return "license_privacy";
  if (/\b(exam|medical exam|no exam)\b/.test(lower)) return "exam";
  return "other";
}
