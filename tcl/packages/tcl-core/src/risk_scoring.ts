...
// ============================================================================
// CONFIGURATION (All thresholds configurable via env or defaults)
// ============================================================================

export interface RiskScoringConfig {
  // Severity thresholds (0-1 risk score maps to severity)
  severityThresholds: {
    critical: number;  // risk >= this → critical
    high: number;      // risk >= this → high
    medium: number;    // risk >= this → medium
    // low is default below medium threshold
  };
  
  // Signal weights (how much each signal contributes to risk)
  weights: {
    claimType: number;
    speaker: number;
    topic: number;
...
export interface RiskSignals {
  ...
  // NLI scores (0-1, higher = stronger)
  maxContradictionScore: number;
  maxSupportScore: number;
  groundingScore: number;  // Max entailment with evidence
  
  // Spectral outputs (0-1)
...
export function extractRiskSignals(
  ...
  analysisResults?: {
    nliScores?: { contradiction: number; support: number; grounding: number };
    spectral?: { nodeBlameNorm: number; truthState: string };
  ...
  const involvedContradictions = contradictions.filter(
  ...
  return {
    ...
    truthState: (analysisResults?.spectral?.truthState as any) ?? "Inconclusive",
...
export function computeRiskScore(
  signals: RiskSignals,
  config: RiskScoringConfig = getDefaultRiskConfig()
): RiskScoreResult {
  const breakdown = {
    claimTypeRisk: 0,
    speakerRisk: 0,
  ...
  if (signals.topicTags.length > 0) {
    breakdown.topicRisk = Math.max(
      ...signals.topicTags.map(t => config.topicRisk[t] ?? 0.2)
    );
  }
  ...
  const riskScore = Math.min(1.0, (
    breakdown.claimTypeRisk * weights.claimType +
    ...
    breakdown.truthStateRisk * weights.truthState
  ) / totalWeight);
...
export type IssueType = 
  | "CONTRADICTION"
  | "UNVERIFIED"      // New: transcript-only, no external docs
  | "UNSUPPORTED"     // External docs present but no support
  | "CIRCULAR"
  | "POLICY_VIOLATION"
  | "POLICY_MISS"
  | "VAGUE_LANGUAGE"
  | "LATE_DISCLAIMER"
  | "PROMISE_RISK"    // New: agent promises that need verification
  | "ABSOLUTE_CLAIM"; // New: claims with absolute language
 ...
 */
export function getIssueTypeExplanation(issueType: IssueType, hasExternalDocs: boolean): string {
  switch (issueType) {
    case "CONTRADICTION":
