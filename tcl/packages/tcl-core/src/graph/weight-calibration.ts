...
import { GraphEdge, ClaimNode, EvidenceNode, EvidenceKind, ClaimModality } from './types.js';
...
export function calibrateEdgeWeight(input: CalibrationInput): CalibrationResult {
...
function computeMethodAgreement(nliScore?: number, heuristicScore?: number): number {
...
function computeModalityWeight(claim: ClaimNode | undefined): number {
  if (!claim) return 0.5;
  
  // Strong modalities get higher weight
  const modalityWeights: Record<ClaimModality, number> = {
    'assert': 0.8,
...
function computeSlotMatchBonus(edge: GraphEdge): number {
...
function computeOverallConfidence(factors: CalibrationFactors): number {
  ...
  return (
    factors.methodAgreement * weights.methodAgreement +
...
export function applyThresholds(
  edges: GraphEdge[],
  thresholds: { support: number; contradiction: number; grounding: number }
): GraphEdge[] {
