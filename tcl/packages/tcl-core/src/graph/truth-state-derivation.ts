...
export function deriveTruthStatesFromGraph(graph: ClaimGraph): TruthDerivationResult {
  const config = getTemplateConfig();
  const truthConfig = config.truthDerivation;
  
  const results: TruthStateResult[] = [];
  const summary = {
    supported: 0,
    contradicted: 0,
    unverified: 0,
  ...
  for (const claim of graph.nodes.claims) {
    const result = deriveTruthStateForClaim(
      claim,
      edgesByFrom,
      edgesByTo,
...
function deriveTruthStateForClaim(
  claim: ClaimNode,
  edgesByFrom: Map<string, GraphEdge[]>,
  edgesByTo: Map<string, GraphEdge[]>,
  evidenceNodes: import('./types.js').EvidenceNode[],
  config: { 
  ...
  // Categorize edges
  const contradictionEdges = allEdges.filter(
    e => e.type === 'CONTRADICTION' && e.weight >= config.minContradictionWeight
  );
  const supportEdges = allEdges.filter(
    e => e.type === 'SUPPORT' && e.weight >= config.minSupportWeight
  );
...
// =============================================================================
// COMPUTE TRUTH SCORES (For spectral.py)
// =============================================================================
...
export function computeTruthScores(derivation: TruthDerivationResult): TruthScores {
  ...
  const auditTruth = Math.round(
    externalVerification * 0.5 +
    consistency * 0.3 +
...
export function applyTruthStatesToClaims(
  claims: ClaimNode[],
  derivation: TruthDerivationResult
): void {
  ...
  for (const claim of claims) {
    ...
    if (result) {
      (claim as any).truthState = result.truthState;
      (claim as any).truthConfidence = result.confidence;
      (claim as any).truthDerivedFrom = result.derivedFrom;
    }
  }
}
