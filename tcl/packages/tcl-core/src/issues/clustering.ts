...
import { getRiskModelConfig, type RiskModelConfig } from "../config/risk.model.js";
...
export function clusterClaims(
  ...
  const claimEdges = new Map<string, Edge[]>();
  ...
  for (const orphan of orphans) {
    if (orphanVisited.has(orphan.id)) continue;
...
function groupByTopics(claims: ClaimForClustering[]): Map<string, string[]> {
...
function createCluster(
  claimIds: string[],
  allClaims: ClaimForClustering[],
  allEdges: Edge[],
  claimMap: Map<string, ClaimForClustering>
): ClaimCluster {
  const claimSet = new Set(claimIds);
  
  // Find edges within this cluster
  const clusterEdges = allEdges.filter(e => 
    claimSet.has(e.fromClaimId) && claimSet.has(e.toClaimId)
  );
  ...
  for (const edge of clusterEdges) {
    if (edge.type === "CONTRADICTION") contradictionMass += edge.score;
    else if (edge.type === "SUPPORT") supportMass += edge.score;
    else if (edge.type === "GROUNDING") groundingMass += edge.score;
  }
...
function generateClusterId(claimIds: string[]): string {
...
function clusterToIssue(
  ...
  const clusterClaims = cluster.claimIds
    ...
    .filter((c): c is ClaimForClustering => c !== undefined);
...
function calculateRiskScore(
  ...
  flags: Issue["flags"],
  config: RiskModelConfig
): { riskScore: number; severity: IssueSeverity; drivers: string[] } {
...
function calculateConfidence(
  cluster: ClaimCluster,
  edges: Edge[]
): { confidence: IssueConfidence; explanation: string } {
  ...
  if (hasContradictions) confidenceScore += 0.25;
  if (hasMultipleClaims) confidenceScore += 0.15;
  if (hasEdges) confidenceScore += 0.1;
...
function calculateCentrality(cluster: ClaimCluster, edges: Edge[]): number {
...
function calculateRecency(cluster: ClaimCluster): number {
...
function selectEvidence(
  ...
  cluster: ClaimCluster,
  config: RiskModelConfig
): { primary: EvidenceSnippet[]; supporting: EvidenceSnippet[] } {
  // Sort claims by turn index for consistent ordering
  const sorted = [...claims].sort((a, b) => a.turnIndex - b.turnIndex);
  
  const primary: EvidenceSnippet[] = [];
  const supporting: EvidenceSnippet[] = [];
