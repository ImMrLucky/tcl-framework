...
export interface EntityPattern {
  type: string;           // e.g., "MONEY", "RATE", "TERM"
  patterns: RegExp[];     // Regex patterns to match
  normalizer?: (match: RegExpMatchArray) => string | number;  // How to normalize
  priority?: number;      // Higher = extracted first
}

export interface SynonymGroup {
  canonical: string;      // The normalized form
  terms: string[];        // All synonyms
}

export interface ActionPattern {
  type: string;           // e.g., "PROMISE", "DECISION", "MODIFICATION"
  patterns: RegExp[];     // Regex patterns to detect
 ...
 */
export const UNIVERSAL_ENTITIES: EntityPattern[] = [
  {
    type: 'MONEY',
    patterns: [
      /\$(\d+(?:,\d{3})*(?:\.\d{2})?)/gi,
  ...
  },
  {
    ...
    normalizer: (match) => {
      ...
      const days: Record<string, number> = { minute: 1/1440, hour: 1/24, day: 1, week: 7, month: 30, year: 365 };
 ...
 */
export const UNIVERSAL_SYNONYMS: SynonymGroup[] = [
 ...
 */
export const UNIVERSAL_ACTIONS: ActionPattern[] = [
  {
    type: 'PROMISE',
    patterns: [
 ...
 */
export function mergeNLPConfig(base: NLPConfig, extension: Partial<NLPConfig>): NLPConfig {
  return {
...
let currentConfig: NLPConfig = DEFAULT_NLP_CONFIG;

/**
 * Set the active NLP config (called by app layer)
 */
export function setNLPConfig(config: Partial<NLPConfig>): void {
 ...
 */
export function resetNLPConfig(): void {
