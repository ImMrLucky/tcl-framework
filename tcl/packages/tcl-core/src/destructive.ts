...
import type { Claim, GroundingEdge, ContradictionEdge, Violation, DestructiveClaim, DestructiveReason } from "./types.js";
...
export function computeDestructiveClaims(args: {
  ...
  const contradictionIncidentSum = new Map<string, number>();
  ...
  const contradictionNorm = new Map<string, number>();
  ...
  const policySeverity = new Map<string, "error" | "warning" | "none">();
  const policyRuleIds = new Map<string, string[]>();
  
  for (const violation of customRuleViolations) {
    if (violation.type === "CUSTOM_RULE" && violation.claimId) {
      const rule = customRuleViolations.find(v => v.type === "CUSTOM_RULE" && (v as any).ruleId);
      ...
      if ((violation as any).ruleId) {
        existing.push((violation as any).ruleId);
      }
  ...
  const confidenceOverall = new Map<string, number>();
  const groundingScore = new Map<string, number>();
  ...
  const destructiveClaims: DestructiveClaim[] = [];
  ...
  for (const claim of claims) {
    ...
    const truthState = spectral?.truthStates?.[idx] as "Supported" | "Contradicted" | "Ungrounded" | "Inconclusive" | undefined;
    ...
    // Build reasons array
    const reasons: Array<{ kind: DestructiveReason; weight: number; detail?: string }> = [];
