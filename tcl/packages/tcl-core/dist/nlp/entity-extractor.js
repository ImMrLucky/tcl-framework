...
import { extractEntitiesSingle, configureSpacyClient } from './spacy-client.js';
 ...
 */
export function configureEntityExtraction(config) {
 ...
 */
export async function extractPrimarySubjectAsync(text) {
    const entities = await extractEntitiesAsync(text);
    return getPrimarySubjectFromEntities(entities);
}
function getPrimarySubjectFromEntities(entities) {
 ...
 */
export async function sharesPrimaryEntityAsync(textA, textB) {
    const entitiesA = await extractEntitiesAsync(textA);
    const entitiesB = await extractEntitiesAsync(textB);
    return checkEntityOverlap(entitiesA, entitiesB);
}
function checkEntityOverlap(entitiesA, entitiesB) {
