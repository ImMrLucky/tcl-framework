import { getIssueScoringConfig } from '../config/issue-scoring.js';
/**
 * Score issues with transcript-only caps and proper ranking
 */
export function scoreIssues(issues, context, config) {
...
function scoreSingleIssue(issue, context, config) {
    ...
    let severityDisplay = impact === 'high' ? 'high' : impact === 'medium' ? 'medium' : 'low';
    const capsApplied = [];
...
function computeDisputeBoost(issue, config) {
    ...
    const hasDispute = issue.compliance.tags?.some(tag => tag.includes('dispute') || tag.includes('denial') || tag.includes('challenge'));
    // Return as point boost (not multiplier)
    return hasDispute ? (config.weights.disputeBoostPoints ?? 6) : 0;
}
function computeContradictionBoost(issue, config) {
...
function computeCommitmentBoost(issue, config) {
    const hasCommitment = issue.compliance.tags?.some(tag => tag.includes('commitment') || tag.includes('promise') || tag.includes('guarantee'));
...
function computeEscalationBoost(issue, config) {
    const text = (issue.what.claimText || issue.what.issueSummary || '').toLowerCase();
    const hasEscalation = config.taxonomy.escalationKeywords.some(keyword => text.includes(keyword.toLowerCase()));
    // Return as point boost
    return hasEscalation ? (config.weights.escalationBoostPoints ?? 8) : 0;
}
function computeTemplateBoost(issue, context, config) {
...
function checkStrictContradiction(issue, config) {
...
function determineRecommendedAction(issue, context, severityDisplay, verificationLevel) {
