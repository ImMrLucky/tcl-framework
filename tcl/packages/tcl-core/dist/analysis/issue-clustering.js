 ...
 */
export function aggregateIssues(atomicIssues, evalMode) {
    ...
    for (const [clusterId, clusterIssues] of Array.from(clusterMap.entries())) {
        ...
        clusterIssues.forEach(i => {
            if (i.what?.primaryClaimId)
                allClaimIds.add(i.what.primaryClaimId);
            if (i.what?.relatedClaimIds)
                i.what.relatedClaimIds.forEach(id => allClaimIds.add(id));
        });
 ...
 */
function computeClusterScore(clusterIssues, evalMode) {
...
function deriveSeverityFromScore(riskScore) {
    if (riskScore >= 0.90)
        return 'critical';
    if (riskScore >= 0.75)
        return 'high';
