...
function requiredForClaim(text) {
    const out = new Set(["transcript_anchor"]);
    for (const hint of EVIDENCE_HINTS) {
        if (hint.patterns.some(p => p.test(text)))
            hint.required.forEach(r => out.add(r));
    }
    return Array.from(out);
}
function statusForClaim(claim, hasExternal, issuesByClaim) {
    const issues = issuesByClaim.get(claim.id) ?? [];
    if (issues.some(i => /false|CONTRADICT|NUMERIC_MISMATCH/i.test(i.type)))
        return "contradicted";
    if (issues.some(i => /PROTECTQA|GUARANTEE|MISLEADING|HALLUCIN/i.test(i.type) && i.severity === "critical"))
        return "false_by_rule";
    ...
    if (externallySupported && hasTranscriptGrounding)
        return "supported";
    if (externallySupported)
        return "partially_supported";
    if (issues.some(i => /UNSUPPORTED|UNVERIFIED|missing evidence/i.test(i.type + (i.what.issueSummary || ""))))
        return "unsupported";
    if (hasTranscriptGrounding)
        return hasExternal ? "unsupported" : "transcript_only";
    return "unverifiable";
}
export function buildEvidenceDependencyGraph(claims, issues, opts) {
    ...
    return claims.map(claim => {
        ...
        let presentEvidenceTypes = ["transcript"];
        if (opts.hasExternalEvidence && claim.truthState === "SUPPORTED")
            presentEvidenceTypes.push("approved_policy_or_doc");
...
export function averageEvidenceSupportScore(nodes, hasExternalEvidence) {
    if (nodes.length === 0)
        return hasExternalEvidence ? 40 : 55;
    let sum = 0;
...
export function evidenceGapCount(nodes) {
