...
function isAgentRole(speakerType) {
    return speakerType === "agent" || speakerType === "supervisor";
}
function buildAgentSignals(claims) {
    return claims
...
export function detectDrift(claims, context) {
    ...
    const maxStrengthByTopic = new Map();
    ...
    for (const signal of agentSignals) {
        if (signal.strength < 0.7 || !DISCLOSURE_REQUIRED_TOPICS.has(signal.topic))
            continue;
        ...
        if (!hasQualifier) {
            driftIssues.push(makeDriftIssue("DISCLOSURE_OMISSION_DRIFT", claimFromSignal(signal), context, `High-commitment claim on '${signal.topic}' (turn ${signal.turnIndex}) was not followed by a qualifier or disclosure within the next 5 turns.`));
...
function makeDriftIssue(type, claim, context, detail) {
    ...
    return {
        ...
        what: {
            ...
            whyItMatters: type === "SPEAKER_ATTRIBUTION_FAILURE"
