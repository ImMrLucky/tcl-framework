 ...
 */
export function detectComplianceIssues(claims, runId, conversationId, evidenceMode) {
    const issues = [];
 ...
 */
function detectCVVStorage(claims, runId, conversationId, evidenceMode) {
    const issues = [];
    const cvvPatterns = [
        ...
        if (hasCVVMention) {
            ...
            const speaker = claim.meta?.speaker === 'Agent' || claim.meta?.speaker === 'AGENT' ? 'AGENT' :
                claim.meta?.speaker === 'Customer' || claim.meta?.speaker === 'CUSTOMER' ? 'CUSTOMER' :
                    'UNKNOWN';
            const verificationLevel = evidenceMode === 'TRANSCRIPT_PLUS_EXTERNAL' ? 'EXTERNAL_VERIFIED' : 'TRANSCRIPT_ONLY';
            issues.push({
 ...
 */
function detectRecordingConsent(claims, runId, conversationId, evidenceMode) {
    const issues = [];
    // Look for agent claims about recording
    const agentClaims = claims.filter(c => c.meta?.speaker === 'Agent' || c.meta?.speaker === 'AGENT');
 ...
 */
function detectPIICollection(claims, runId, conversationId, evidenceMode) {
    const issues = [];
