const PATTERNS = [
    {
        ...
        summary: (m) => `Health factor mentioned (${m.trim().slice(0, 80)}) — underwriting relevance`,
    ...
    },
    {
        ...
        summary: (m) => `Customer objection signal: "${m.trim().slice(0, 100)}"`,
...
export function mineBusinessInsights(claims) {
    const insights = [];
    for (const claim of claims) {
        if (claim.meta?.speakerType !== "customer")
            continue;
