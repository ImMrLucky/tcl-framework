 ...
 */
export function collapseIssuesToClusters(atomicIssues) {
    ...
    for (const [clusterId, clusterIssues] of Array.from(clusterMap.entries())) {
        ...
        const highestRiskIssue = clusterIssues.reduce((max, issue) => (issue.riskScore || 0) > (max.riskScore || 0) ? issue : max);
        const clusterSeverity = highestRiskIssue.severity || 'low';
        // 4) verification: If any atomic issue is externally verified, mark cluster as verified
        const verificationLevels = clusterIssues
        ...
        const representativeIssue = clusterIssues.reduce((best, issue) => {
            ...
            if (issuePriority > bestPriority)
                return issue;
            if (issuePriority === bestPriority && (issue.riskScore || 0) > (best.riskScore || 0))
                return issue;
            return best;
        });
        // 6) Aggregate involvedClaimIds and turnIndexes
        const involvedClaimIds = new Set();
        const involvedTurnIndexes = new Set();
        ...
        const topicIds = clusterIssues
            .map(i => i.topicId)
            .filter((id) => id !== undefined && id !== 'unknown');
        const slotKeys = clusterIssues
            .map(i => i.slotKey)
            .filter((key) => key !== undefined && key !== 'unknown');
        ...
        const grouped = {
            ...
            what: {
                ...
                relatedClaimIds: Array.from(involvedClaimIds).filter(id => id !== representativeIssue.what?.primaryClaimId),
            },
            rollup: {
