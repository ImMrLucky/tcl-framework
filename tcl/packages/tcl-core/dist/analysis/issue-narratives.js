 ...
 */
export function buildIssueNarratives(input) {
    ...
    const clusters = clusterClaimsIntoIssues(input.claims, input.contradictions, input.supports, input.grounding, input.spectral, config);
    ...
    for (const cluster of clusters) {
        const narrative = buildNarrativeForCluster(cluster, claimMap, input.transcript || "", templates, taxonomy, config, input.spectral, input.claims, evidenceMode, groundedClaimIds);
        if (narrative) {
...
function clusterClaimsIntoIssues(claims, contradictions, supports, grounding, spectral, config) {
    const clusters = [];
...
// NARRATIVE BUILDING
// ============================================================================
function buildNarrativeForCluster(cluster, claimMap, transcript, templates, taxonomy, config, spectral, allClaims, evidenceMode = 'TRANSCRIPT_ONLY', groundedClaimIds = new Set()) {
    const claims = cluster.claimIds.map(id => claimMap.get(id)).filter(Boolean);
    if (claims.length === 0) {
        return null;
    }
    ...
    const title = substituteTemplate(templates.titles[cluster.issueType] || templates.titles.generic, templateVars);
    const whatIsWrong = substituteTemplate(templates.whatIsWrong[cluster.issueType] || templates.whatIsWrong.generic, templateVars);
    const whyWrong = (templates.whyWrong[cluster.issueType] || templates.whyWrong.generic).map((t) => substituteTemplate(t, templateVars));
    const whyItMatters = (templates.whyItMatters[cluster.issueType] || templates.whyItMatters.generic).map((t) => substituteTemplate(t, templateVars));
    ...
    const supportBasis = hasGrounding
        ? (evidenceMode === 'TRANSCRIPT_PLUS_EXTERNAL' ? 'EXTERNAL' : 'TRANSCRIPT')
        : 'NONE';
    const verificationLevel = evidenceMode === 'TRANSCRIPT_PLUS_EXTERNAL'
        ? 'EXTERNALLY_VERIFIED'
        : 'TRANSCRIPT_ONLY';
    return {
...
// HELPER FUNCTIONS
// ============================================================================
function extractEvidenceQuotes(claims, transcript) {
    const quotes = [];
    for (const claim of claims) {
...
function buildContradictionPairs(contradictions, quotes) {
    const pairs = [];
...
function computeIssueScores(cluster, clusterClaims, allClaims, spectral, taxonomy, config) {
            ...
            for (const claim of clusterClaims) {
                ...
                if (idx !== undefined && idx < spectral.truthStates.length) {
                    ...
                    if (state === "Contradicted")
                        hasContradictedState = true;
                    if (state === "Ungrounded")
                        hasUngroundedState = true;
                }
            }
        }
    ...
    const compositeScore = riskScore * weights.risk +
...
function buildTraceabilityEdges(cluster, claims) {
    const edges = [];
...
function inferSubcategory(claims) {
    const text = claims.map(c => c.text.toLowerCase()).join(" ");
