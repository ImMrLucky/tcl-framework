 ...
 */
export function computeImpactSeverity(type, category, signals) {
    ...
    if (signals.complianceFlags && signals.complianceFlags.length > 0) {
        const hasCriticalFlag = signals.complianceFlags.some(flag => impactRules.criticalComplianceFlags.includes(flag));
        if (hasCriticalFlag) {
            return 'critical';
        }
    }
