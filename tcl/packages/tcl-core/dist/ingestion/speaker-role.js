...
function cleanSpeaker(rawSpeaker) {
    return rawSpeaker
        .trim()
        .toLowerCase()
        .replace(/[_-]+/g, " ")
...
function hasExactLabel(cleaned, labels) {
    return labels.find(label => cleaned === label);
}
function hasRoleLabel(cleaned, labels) {
    return labels.find(label => cleaned === label ||
        cleaned.startsWith(`${label} `) ||
        cleaned.endsWith(` ${label}`) ||
        cleaned.includes(` ${label} `));
}
export function mapSpeakerToRole(rawSpeaker, context) {
...
export function speakerRoleToDisplay(role) {
    switch (role) {
        case "agent":
            return "Agent";
        case "customer":
...
export function isRecognizedTranscriptSpeaker(rawSpeaker) {
