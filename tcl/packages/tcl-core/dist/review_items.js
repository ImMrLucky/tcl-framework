 ...
 */
function reviewItemsFromContradictions(contradictions, claims, config = getScoringConfig()) {
    const items = [];
    ...
    for (const contra of topK) {
        ...
        let severity = "medium";
        if (contra.weight >= 0.8)
            severity = "critical";
        else if (contra.weight >= 0.6)
            severity = "high";
        else if (contra.weight < 0.4)
            severity = "low";
        items.push({
 ...
 */
function reviewItemsFromDestructiveClaims(destructiveClaims, claims, config = getScoringConfig()) {
    const items = [];
    const claimMap = new Map(claims.map(c => [c.id, c]));
    ...
    for (const dc of topK) {
        ...
        // Determine severity
        let severity = "medium";
        if (dc.importance >= 0.8)
            severity = "critical";
        else if (dc.importance >= 0.6)
            severity = "high";
        else if (dc.importance < 0.4)
            severity = "low";
 ...
 */
function reviewItemsFromUngroundedPromises(claims) {
    const items = [];
    // Find agent promises that aren't grounded
    const ungroundedPromises = claims.filter(c => c.claimKind === "promise" &&
...
export function generateReviewItems(input) {
    ...
    allItems.push(...reviewItemsFromContradictions(input.contradictions, input.claims, config));
    // 2. From destructive claims
    if (input.destructiveClaims) {
        allItems.push(...reviewItemsFromDestructiveClaims(input.destructiveClaims, input.claims, config));
    }
...
// HELPERS
// ============================================================================
function generateReviewId(...parts) {
    const hash = createHash("sha256").update(parts.join(":")).digest("hex").substring(0, 12);
    return `review_${hash}`;
}
function detectContradictionCategory(textA, textB) {
