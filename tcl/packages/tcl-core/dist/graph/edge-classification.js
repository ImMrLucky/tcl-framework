...
import { areSlotsEquivalent, isHardSlot } from './slot-registry.js';
// =============================================================================
// MAIN CLASSIFICATION FUNCTION
// =============================================================================
export function classifyEdges(contradictionCandidates, supportClaimCandidates, supportEvidenceCandidates, groundingCandidates) {
    ...
    for (const candidate of contradictionCandidates) {
        ...
        if (result.rejected) {
            ...
            if (result.reason === 'topic')
                rejectedByTopicGating++;
            if (result.reason === 'polarity')
                rejectedByPolarityGating++;
            if (result.reason === 'threshold')
                rejectedByThreshold++;
            if (result.reason === 'ineligible_slot')
                rejectedByIneligibleSlot++;
            if (result.reason === 'value_type_mismatch')
    ...
    return {
        ...
        diagnostics: {
            candidatesProcessed: contradictionCandidates.length +
                supportClaimCandidates.length +
                supportEvidenceCandidates.length +
                groundingCandidates.length,
            edgesCreated: deduplicatedContradictions.length +
...
function hasSharedSubjectReference(a, b) {
 ...
 */
function valueTypesCompatible(a, b) {
...
function hasOpposingPolarity(a, b) {
    // Check modality opposition
    if (a.modality === 'assert' && b.modality === 'deny')
        return true;
    if (a.modality === 'deny' && b.modality === 'assert')
        return true;
    const aText = a.text.toLowerCase();
    const bText = b.text.toLowerCase();
...
// =============================================================================
function computeContradictionScore(a, b, signals) {
    const config = getTemplateConfig();
...
// =============================================================================
function classifyClaimSupport(candidate, config) {
    const { claimA, claimB, signals } = candidate;
...
// =============================================================================
function classifyGrounding(candidate, config) {
    const { claim, evidence, signals } = candidate;
...
// =============================================================================
function computeSupportScore(a, b, signals) {
...
function computeEvidenceSupportScore(claim, evidence, signals) {
...
// CREATE EDGES
// =============================================================================
function createSupportEdge(fromId, toId, weight, signals, sourceType) {
    return {
...
function createGroundingEdge(claim, evidence, weight, signals) {
    return {
