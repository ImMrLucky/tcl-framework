...
export class PlanService {
     ...
     */
    async getOrgPlanContext(orgId, emulation) {
        if (!supabaseAdmin) {
        ...
        const realTier = (org.plan_tier || 'SANDBOX');
        const status = (org.plan_status || 'ACTIVE');
        // Check if emulation is active
        const isEmulated = emulation?.enabled === true && emulation?.planTier;
        const effectiveTier = isEmulated ? emulation.planTier : realTier;
        ...
        if (org.features_override_json) {
            const override = org.features_override_json;
            if (override.enabled) {
            ...
            if (override.disabled) {
                ...
                capabilities = capabilities.filter((cap) => !override.disabled.includes(cap));
            }
        }
        ...
        const remainingToday = {
            ...
            uploads: limits.uploadsPerDay === -1
                ? -1
     ...
     */
    async consumeUsage(orgId, metric, amount = 1) {
        // Get plan context
        ...
        const { data: usageRows, error: usageError } = await supabaseAdmin.rpc('get_or_create_usage_today', { p_org_id: orgId });
        if (usageError) {
     ...
     */
    async recordUsage(orgId, metric, amount) {
        if (!supabaseAdmin) {
            throw new Error('Supabase not configured');
        }
        if (!supabaseAdmin) {
     ...
     */
    async hasCapability(orgId, capability) {
        const context = await this.getOrgPlanContext(orgId);
