 ...
 */
export function setupIssueDecisionsRoutes(app) {
    // ============================================================================
    ...
    // ============================================================================
    app.post('/api/issues/:issueId/decision', requireEntitlement('issueDecisions'), async (req, res) => {
        try {
            const context = await getOrgContext(req);
            ...
            const { issueId } = req.params;
            const { disposition, severityOverride, assignedToUserId, notes, expiresAt, evaluationId, projectId, } = req.body;
            if (!disposition) {
            ...
            if (projectId)
                decisionData.project_id = projectId;
            if (evaluationId)
                decisionData.evaluation_id = evaluationId;
            if (severityOverride)
                decisionData.severity_override = severityOverride;
            if (assignedToUserId)
                decisionData.assigned_to_user_id = assignedToUserId;
            if (notes)
                decisionData.notes = notes;
            if (expiresAt)
                decisionData.expires_at = expiresAt;
            let decision;
            ...
            try {
                ...
                if (webhookIntegration) {
                    const config = webhookIntegration.config_json;
                    const enabledEvents = config.enabled_events || [];
                    if (enabledEvents.includes('issue.decision.updated') || enabledEvents.includes('*')) {
                        await deliverWebhook(webhookIntegration.id, context.orgId, 'issue.decision.updated', {
    ...
    // ============================================================================
    app.post('/api/issues/bulk-decisions', requireEntitlement('issueDecisions'), async (req, res) => {
        try {
            const context = await getOrgContext(req);
            if (!context || context.error || !context.orgId || !context.userId) {
                return res.status(401).json({ error: context?.error || 'Authorization required' });
