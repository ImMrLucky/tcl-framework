 ...
 */
export function setupIssueSnapshotsRoutes(app) {
    // ============================================================================
    ...
    // ============================================================================
    app.post('/api/issues/:issueId/lock', requireEntitlement('legalHold'), async (req, res) => {
        try {
            const context = await getOrgContext(req);
            ...
            try {
                ...
                if (webhookIntegration) {
                    ...
                    if (enabledEvents.includes('issue.locked') || enabledEvents.includes('*')) {
                        await deliverWebhook(webhookIntegration.id, context.orgId, 'issue.locked', {
    ...
    // ============================================================================
    app.post('/api/issues/:issueId/unlock', requireEntitlement('legalHold'), async (req, res) => {
        try {
            const context = await getOrgContext(req);
            if (!context || context.error || !context.orgId || !context.userId) {
                return res.status(401).json({ error: context?.error || 'Authorization required' });
