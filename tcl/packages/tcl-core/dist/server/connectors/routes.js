 ...
 */
function getConnectorProvider(type) {
 ...
 */
async function getConnectorSecrets(orgId, connectorType, supabase) {
 ...
 */
export function setupConnectorRoutes(app) {
    // ============================================================================
    // POST /api/connectors/:type/test - Test connector connection
    // ============================================================================
    app.post('/api/connectors/:type/test', requireEntitlement('batchIngestion'), async (req, res) => {
        try {
    ...
    // ============================================================================
    app.get('/api/connectors/:type/list', requireEntitlement('batchIngestion'), async (req, res) => {
        try {
            const context = await getOrgContext(req);
            ...
            if (ALLOW_DEV_CONNECTOR_SECRETS_FROM_CLIENT && req.query.secrets) {
                ...
                try {
                    secrets = JSON.parse(req.query.secrets);
                }
                catch (parseError) {
            ...
            const result = await connector.list({ path, prefix, limit, offset, recursive }, config, secrets || {});
            res.json(result);
        }
        catch (error) {
    ...
    // ============================================================================
    app.post('/api/connectors/:type/batch-from-selection', requireEntitlement('batchIngestion'), async (req, res) => {
        try {
            const context = await getOrgContext(req);
            if (!context || context.error || !context.orgId || !context.userId) {
                return res.status(401).json({ error: context?.error || 'Authorization required' });
            ...
            const batchItems = selection.map((obj) => ({
                batch_id: batch.id,
                status: 'PENDING',
    ...
    // ============================================================================
    app.get('/api/connectors/:type/status', requireEntitlement('batchIngestion'), async (req, res) => {
        try {
            const context = await getOrgContext(req);
            if (!context || context.error || !context.orgId) {
                return res.status(401).json({ error: context?.error || 'Authorization required' });
    ...
    // ============================================================================
    app.post('/api/connectors/s3/connect', requireEntitlement('batchIngestion'), async (req, res) => {
        try {
            const context = await getOrgContext(req);
            if (!context || context.error || !context.orgId || !context.userId) {
                return res.status(401).json({ error: context?.error || 'Authorization required' });
            ...
            if (connector) {
                const testResult = await connector.testConnection({ bucket, region, prefix: prefix || '' }, mode === 'ASSUME_ROLE'
                    ? { roleArn, externalId }
