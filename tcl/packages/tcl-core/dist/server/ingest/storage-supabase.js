 ...
 */
export async function computeSha256FromFile(filePath) {
 ...
 */
export async function uploadFileToSupabase({ bucket, objectPath, filePath, contentType, }) {
    if (!supabaseAdmin) {
 ...
 */
async function uploadFileStreaming(bucket, objectPath, filePath, contentType, fileSize) {
 ...
 */
export async function downloadFileFromSupabase(bucket, objectPath) {
    if (!supabaseAdmin) {
 ...
 */
export async function createSignedUrl(bucket, objectPath, expiresIn = 300 // 5 minutes default
) {
    if (!supabaseAdmin) {
        throw new Error('Supabase not configured');
    }
