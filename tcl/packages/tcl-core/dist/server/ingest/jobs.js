...
function ensureUploadTempDirSync() {
    try {
 ...
 */
export async function uploadJobFiles(jobId, orgId, uploaderUserId, audioFile, transcriptFile) {
    if (!supabaseAdmin) {
    ...
    if (transcriptFile?.path)
        tempFiles.push(transcriptFile.path);
    try {
...
export async function getJobStatus(jobId, orgId) {
    ...
    const audioAssetId = job.audio_asset_id;
    const transcriptAssetId = job.transcript_asset_id;
    return {
        jobId: job.id,
        status: job.status,
...
export function registerIngestionJobRoutes(app) {
    ...
    // Upload files
    app.post('/api/ingest/jobs/:jobId/upload', (req, res, next) => {
        ...
        next();
    }, requireCapability(Capability.ANALYZE_MANUAL_UPLOAD), (req, res, next) => {
    ...
    app.post('/api/ingest/jobs/:jobId/upload-metadata', requireCapability(Capability.ANALYZE_MANUAL_UPLOAD), async (req, res) => {
        try {
            const { jobId } = req.params;
    ...
    app.post('/api/ingest/jobs/:jobId/finalize-upload', requireCapability(Capability.ANALYZE_MANUAL_UPLOAD), async (req, res) => {
        try {
            const context = await getOrgContext(req);
            if (!context || context.error) {
                return res.status(401).json({ error: context?.error || 'Authorization required' });
            ...
            const { jobId } = req.params;
            const { assetId, bucket, objectPath, filename, sizeBytes, sha256, kind } = req.body;
            // Validate inputs
            ...
            if (!fetchError && updatedJob) {
                ...
                let newStatus = updatedJob.status;
    ...
    app.post('/api/ingest/jobs/:jobId/start', requireCapability(Capability.ANALYZE_MANUAL_UPLOAD), async (req, res) => {
        try {
            const context = await getOrgContext(req);
            if (!context || context.error) {
                return res.status(401).json({ error: context?.error || 'Authorization required' });
    ...
    app.get('/api/ingest/assets/:assetId/signed-url', requireCapability(Capability.ANALYZE_MANUAL_UPLOAD), async (req, res) => {
        try {
            const context = await getOrgContext(req);
            if (!context || context.error) {
                return res.status(401).json({ error: context?.error || 'Authorization required' });
