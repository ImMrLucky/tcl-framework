...
import { computeSha256FromFile, uploadFileToSupabase, getMimeType } from '../ingest/storage-supabase.js';
 ...
 */
export async function storeEvidenceFile(filePath, orgId, evidenceId, scope, originalName) {
    if (!supabaseAdmin) {
 ...
 */
export async function storeEvidenceFileFromBuffer(buffer, orgId, evidenceId, scope, originalName) {
    ...
    try {
        ...
        const result = await storeEvidenceFile(tempFilePath, orgId, evidenceId, scope, originalName);
        return result;
    }
    finally {
        // Clean up temp file
 ...
 */
export function computeSha256FromBuffer(buffer) {
 ...
 */
export async function snapshotEvidenceLink(url, orgId, evidenceId) {
    if (!supabaseAdmin) {
        throw new Error('Supabase not configured');
    }
 ...
 */
export async function downloadEvidenceFile(storagePath) {
    if (!supabaseAdmin) {
        throw new Error('Supabase not configured');
    }
 ...
 */
export function getEvidenceFilePublicUrl(storagePath) {
    if (!supabaseAdmin) {
        return null;
    }
 ...
 */
export async function createEvidenceFileSignedUrl(storagePath, expiresIn = 3600 // 1 hour default
) {
    if (!supabaseAdmin) {
        throw new Error('Supabase not configured');
    }
