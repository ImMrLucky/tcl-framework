 ...
 */
export async function getEvidenceItemById(evidenceItemId, orgId) {
    if (!supabaseAdmin) {
        throw new Error('Supabase not configured');
    }
    const { data, error } = await supabaseAdmin
...
export async function listEvidenceItems(options) {
    if (!supabaseAdmin) {
        throw new Error('Supabase not configured');
    }
 ...
 */
export async function updateEvidenceItem(evidenceItemId, orgId, input, updatedBy) {
    if (!supabaseAdmin) {
        throw new Error('Supabase not configured');
    }
    ...
    if (input.regions !== undefined)
        updateData.regions = input.regions;
    ...
    if (input.ruleMeta !== undefined)
        updateData.rule_meta = input.ruleMeta;
    // Authority & Override (only for ORG scope)
 ...
 */
export async function approveEvidenceItem(evidenceItemId, orgId, approvedBy) {
    if (!supabaseAdmin) {
        throw new Error('Supabase not configured');
    }
    const { data, error } = await supabaseAdmin
 ...
 */
export async function deprecateEvidenceItem(evidenceItemId, orgId, deprecatedBy, notes) {
    if (!supabaseAdmin) {
        throw new Error('Supabase not configured');
    }
    const { data, error } = await supabaseAdmin
 ...
 */
export async function updateIndexingStatus(evidenceItemId, status, chunkCount, embeddingModel, indexError) {
    if (!supabaseAdmin) {
        throw new Error('Supabase not configured');
    }
    const updateData = {
 ...
 */
export async function resolveEvidenceSet(orgId, projectId, templateId, conversationId, simulationMode = false, includeOrg = true, includeProject = true, includeTemplate = true) {
    if (!supabaseAdmin) {
        throw new Error('Supabase not configured');
    }
 ...
 */
function mapDbRowToEvidenceItem(row) {
    return {
        id: row.id,
        orgId: row.org_id,
