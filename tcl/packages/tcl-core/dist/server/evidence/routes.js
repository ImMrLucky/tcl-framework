...
import { supabaseAdmin, getUserRole } from '../supabase.js';
import { getOrgContext } from '../auth-context.js';
import { createEvidenceItem, getEvidenceItemById, listEvidenceItems, updateEvidenceItem, approveEvidenceItem, deprecateEvidenceItem, updateIndexingStatus, resolveEvidenceSet, } from './service.js';
import { storeEvidenceFile, snapshotEvidenceLink, createEvidenceFileSignedUrl, } from './storage.js';
...
export function setupEvidenceRoutes(app) {
    ...
    app.post('/api/evidence/upload', upload.single('file'), async (req, res) => {
        try {
            ...
            const { scope, sourceType, title, description, tags, regions, projectId, conversationId, templateId, status, version, effectiveFrom, effectiveTo, } = req.body;
            ...
            const storageResult = await storeEvidenceFile(req.file.path, context.orgId, evidenceId, scope, req.file.originalname);
    ...
    app.post('/api/evidence/link', async (req, res) => {
        try {
            ...
            const { url, scope, sourceType, title, description, tags, regions, projectId, conversationId, templateId, snapshotNow, status, version, effectiveFrom, effectiveTo, } = req.body;
    ...
    app.get('/api/evidence', async (req, res) => {
        try {
            ...
            const { projectId, conversationId, templateId, scope, sourceType, status, indexStatus, tags, limit, offset, } = req.query;
    ...
    app.patch('/api/evidence/:id', async (req, res) => {
        try {
            ...
            const { id } = req.params;
            const { title, description, tags, regions, status, version, effectiveFrom, effectiveTo, ruleMeta, } = req.body;
            const evidenceItem = await updateEvidenceItem(id, context.orgId, {
                title,
                description,
    ...
    app.get('/api/evidence/resolve', async (req, res) => {
        try {
            ...
            const { projectId, templateId, conversationId, simulationMode, includeOrg, includeProject, includeTemplate, } = req.query;
            // Normalize simulationMode to boolean
            const isSimulationMode = simulationMode === 'true' ||
                simulationMode === '1' ||
                (typeof simulationMode === 'boolean' && simulationMode) ||
                (typeof simulationMode === 'number' && simulationMode === 1);
            const evidenceSet = await resolveEvidenceSet(context.orgId, projectId || context.projectId, templateId, conversationId, isSimulationMode, includeOrg !== 'false', includeProject !== 'false', includeTemplate !== 'false');
    ...
    app.post('/api/evidence/migrate-policies', async (req, res) => {
        try {
            ...
            const evidenceItems = policiesToMigrate.map((policy) => ({
