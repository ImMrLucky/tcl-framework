 ...
 */
async function processPendingEvidenceItems() {
    ...
    try {
        ...
        for (const item of pendingItems) {
            try {
                ...
                // Mark as failed
                await updateIndexingStatus(item.id, 'FAILED', 0, // chunkCount
 ...
 */
async function processEvidenceItem(item) {
    if (!supabaseAdmin) {
    ...
    try {
        ...
        await updateIndexingStatus(item.id, 'INDEXED', chunks.length, // chunkCount
