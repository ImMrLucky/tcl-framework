 ...
 */
export async function createConversationFromCanonical(orgId, projectId, env, userId, transcript, options) {
    if (!supabaseAdmin) {
    ...
    if (options?.autoAnalyze) {
        try {
            ...
            const jobId = await createIngestionJob(orgId, projectId || '', env, userId, 'TRANSCRIPT_ONLY', options?.title || transcript.source.file_name, options?.channel || 'call', options?.representativeId || null);
 ...
 */
export async function createConversationsFromCanonicalBatch(orgId, projectId, env, userId, transcripts, options) {
    const results = [];
    for (const transcript of transcripts) {
        try {
            const result = await createConversationFromCanonical(orgId, projectId, env, userId, transcript, {
                ...options,
