 ...
 */
export function setupBatchUploadRoutes(app) {
    // ============================================================================
    // POST /api/ingest/batch - Batch file upload with parsing
    // ============================================================================
    app.post('/api/ingest/batch', requireEntitlement('batchIngestion'), batchUpload.array('files', 100), // Allow up to 100 files
    async (req, res) => {
        try {
            ...
            for (const file of files) {
                try {
                    ...
                    if (ext === 'zip') {
                        ...
                        parseResult.transcripts.forEach((t) => {
                            allTranscripts.push({
                                transcript: t,
                        ...
                        parseResult.transcripts.forEach((t, idx) => {
                            importItems.push({
            ...
            if (allTranscripts.length > 0) {
                try {
                    const results = await createConversationsFromCanonicalBatch(context.orgId, context.projectId || '', context.env || 'sandbox', context.userId, allTranscripts.map(t => t.transcript), {
                    ...
                    for (let i = 0; i < results.length; i++) {
                        ...
                        if (result.conversation_id) {
                            ...
                            const importItem = importItems.find(item => item.source_name === sourceName ||
