...
export class TxtSpeakerPrefixedNormalizer {
    ...
    async normalize(content, options) {
        ...
        for (let lineIdx = 0; lineIdx < lines.length; lineIdx++) {
            ...
            if (parsed) {
                // Flush previous turn
                if (currentTurn) {
                    const turn = this.createTurn(currentTurn, turns.length, participantsMap, options, charOffset - currentTurn.text.length);
                    turns.push(turn);
                }
                // Start new turn
                currentTurn = {
    ...
    createTurn(raw, turnIndex, participantsMap, options, charEnd) {
        // Check for overrides
