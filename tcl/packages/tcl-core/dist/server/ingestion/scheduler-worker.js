        ...
        for (const obj of newObjects) {
            try {
                ...
                if (transcripts.length > 0) {
                    ...
                    const results = await createConversationsFromCanonicalBatch(schedule.org_id, '', // projectId - will need to get from schedule config
                    'sandbox', // env - will need to get from schedule config
                    schedule.created_by_user_id, transcripts, {
 ...
 */
async function listNewObjectsFromSource(source, schedule) {
    if (!supabaseAdmin) {
        throw new Error('Database not configured');
    }
    ...
    try {
        ...
        const listResult = await connector.list({ prefix: source.config_json.prefix || '', recursive: true }, config, secrets);
