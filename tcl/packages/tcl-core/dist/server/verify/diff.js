 ...
 */
export async function computeVerificationDiff(orgId, jobId, uploadedTranscriptAssetId, asrTranscriptAssetId, uploadedText, asrText) {
 ...
 */
function computeMismatchScore(text1, text2) {
 ...
 */
function detectEntityMismatches(text1, text2) {
    const mismatches = [];
...
function findClosestMatch(value, candidates) {
    ...
    const fuzzy = candidates.find(c => c.value.toLowerCase().replace(/[^\w]/g, '') === normalized);
    if (fuzzy)
        return fuzzy;
    return null;
}
 ...
 */
function detectHighRiskDifferences(text1, text2) {
    const differences = [];
