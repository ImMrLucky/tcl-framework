 ...
 * Export Claims CSV
 */
export async function exportClaimsCSV(evaluationId, orgId, projectId, env, supabaseAdmin) {
    // Fetch evaluation
    ...
    const rows = claims.map((claim) => {
        const issue = report.issues?.find((i) => i.claimId === claim.id);
        return [
            claim.id,
 ...
 */
export async function exportRunJSON(evaluationId, orgId, projectId, env, supabaseAdmin) {
    // Fetch evaluation
    const { data: evaluation, error: evalError } = await supabaseAdmin
        .from('evaluations')
        .select('*')
 ...
 */
export async function exportIssuePDF(evaluationId, claimId, orgId, projectId, env, supabaseAdmin) {
    // Fetch evaluation
    const { data: evaluation, error: evalError } = await supabaseAdmin
        .from('evaluations')
        .select('report')
    ...
    const report = evaluation.report;
    const issue = report.issues?.find((i) => i.claimId === claimId);
    const claim = report.inputs?.claims?.find((c) => c.id === claimId);
    if (!issue || !claim) {
...
function buildPDFContent(params) {
    const { orgId, projectId, env, evaluationId, createdAt, engineVersion, codeVersion, modelFingerprint, inputHash, configHash, claim, issue, transcript } = params;
    ...
    const lines = [
        ...
        'RELATED EVIDENCE',
        '-'.repeat(50),
        ...(issue.relatedEdges?.topBadContradictions?.slice(0, 3).map((e) => `Contradiction: ${e.claimAId} <-> ${e.claimBId} (weight: ${e.weight?.toFixed(3)})`) || []),
        ...(issue.relatedEdges?.topBadSupports?.slice(0, 3).map((e) => `Support: ${e.claimAId} <-> ${e.claimBId} (weight: ${e.weight?.toFixed(3)})`) || []),
        '',
        'PRIMARY EVIDENCE',
        '-'.repeat(50),
