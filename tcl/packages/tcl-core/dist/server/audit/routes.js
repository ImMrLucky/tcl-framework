...
export function setupAuditRoutes(app) {
    ...
    app.post("/api/conversations/ingest", async (req, res) => {
        ...
        try {
            ...
            await processArtifacts(context.orgId, context.projectId || '', context.env, conversation.id, artifacts);
    ...
    app.get("/api/evaluations/:id", async (req, res) => {
        try {
            ...
            const version = req.query.v; // '2' for v2 API
    ...
    app.post("/api/exports/claims-csv", async (req, res) => {
        try {
            ...
            const result = await exportClaimsCSV(evaluation_id, context.orgId, context.projectId || '', context.env, supabaseAdmin);
            // Log audit
            await logAudit({
                orgId: context.orgId,
    ...
    app.post("/api/exports/run-json", async (req, res) => {
        try {
            ...
            const result = await exportRunJSON(evaluation_id, context.orgId, context.projectId || '', context.env, supabaseAdmin);
            // Log audit
            await logAudit({
                orgId: context.orgId,
                action: 'EXPORT_CREATED',
    ...
    app.post("/api/exports/issue-pdf", async (req, res) => {
        try {
            ...
            const result = await exportIssuePDF(evaluation_id, claim_id, context.orgId, context.projectId || '', context.env, supabaseAdmin);
            // Log audit
            await logAudit({
                orgId: context.orgId,
                action: 'EXPORT_CREATED',
