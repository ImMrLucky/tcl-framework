...
export function setupOrgRoutes(app) {
    // ============================================================================
    ...
    app.patch('/api/orgs/:orgId', async (req, res) => {
        try {
            ...
            const hasPermission = await checkUserPermission(context.userId, orgId, 'configure');
            if (!hasPermission) {
            ...
            const { name, businessFunctionPrimary, industryPrimary, regions, defaultLens, defaultEvidenceInclusion, } = req.body;
            const updateData = {};
            if (name !== undefined)
    ...
    app.get('/api/orgs/:orgId/members', async (req, res) => {
        try {
            ...
            const membersResponse = membersList.map((m) => {
                try {
                    if (!m || !m.user_id) {
