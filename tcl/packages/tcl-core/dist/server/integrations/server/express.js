...
app.post('/v1/realtime/sessions/start', async (req, res) => {
    try {
        const body = req.body;
        const orgContext = await getOrgContext(req.headers.authorization?.replace('Bearer ', ''), undefined // TODO: Extract user ID from JWT
        );
        if (!orgContext) {
...
app.get('/integrations', async (req, res) => {
    try {
        const orgContext = await getOrgContext(req.headers.authorization?.replace('Bearer ', ''), undefined);
        if (!orgContext) {
            return res.status(401).json({ error: 'Unauthorized' });
        }
