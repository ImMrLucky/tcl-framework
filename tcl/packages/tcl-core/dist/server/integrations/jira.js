 ...
 */
export async function getJiraIntegration(orgId, supabase) {
 ...
 */
export async function getJiraCredentials(orgId, supabase) {
 ...
 * Create Jira ticket
 */
export async function createJiraTicket(config, credentials, issue, evaluationLink) {
    ...
    if (config.components && config.components.length > 0) {
        jiraIssue.fields.components = config.components.map((name) => ({ name }));
    }
    // Make API request
 ...
 */
export async function createJiraTicketsBulk(config, credentials, issues, evaluationLink) {
    const tickets = [];
