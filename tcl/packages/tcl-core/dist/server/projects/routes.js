...
export function setupProjectRoutes(app) {
    ...
    app.patch('/api/projects/:projectId', async (req, res) => {
        try {
            ...
            const hasPermission = await checkUserPermission(context.userId, context.orgId, 'configure');
            if (!hasPermission) {
                return res.status(403).json({ error: 'Insufficient permissions. Admin or owner role required.' });
            }
            const { name, description, businessFunctionOverride, industryOverride, defaultTemplateId, defaultLens, } = req.body;
            const updateData = {};
            if (name !== undefined)
