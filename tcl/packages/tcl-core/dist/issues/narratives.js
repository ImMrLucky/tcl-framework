 ...
 */
export function buildIssueNarratives(clusters, claims, edges, spectralData) {
    ...
    for (let i = 0; i < clusters.length; i++) {
        ...
        const contradictionPairs = buildContradictionPairs(cluster.topContradictions, quoteMap, clusterClaims);
        // Determine speaker focus (default: AGENT)
        const speakerFocus = determineSpeakerFocus(clusterClaims);
        // Compute scores
        const scores = computeIssueScores(cluster, clusterClaims, edges, taxonomyConfig, scoringConfig, spectralData);
 ...
 */
function determineCategory(claims, taxonomy) {
 ...
 */
function determineIssueType(cluster, claims, edges) {
 ...
 */
function getTemplateForType(type, templates) {
    return templates[type] || templates.default;
}
/**
 * Build contradiction pairs with explanations
 */
function buildContradictionPairs(topContradictions, quoteMap, claims) {
 ...
 * Determine speaker focus (default: AGENT)
 */
function determineSpeakerFocus(claims) {
    const agentCount = claims.filter(c => c.meta?.speaker === 'Agent' || c.meta?.speaker === 'AGENT').length;
    if (agentCount > 0) {
        return 'AGENT'; // Default: agent-focused
    }
    const systemCount = claims.filter(c => c.meta?.speaker === 'System' || c.meta?.speaker === 'SYSTEM').length;
    if (systemCount > 0) {
        return 'SYSTEM';
    }
    return 'CUSTOMER';
 ...
 */
function computeIssueScores(cluster, claims, edges, taxonomy, scoring, spectralData) {
    ...
    const riskScore = Math.min(100, severityBase * scoring.weights.riskScore.severity +
    ...
    const impactScore = Math.min(100, (cluster.category ? 50 : 30) * scoring.weights.impactScore.category +
    ...
    const fixabilityScore = Math.min(100, clarity * 100 * scoring.weights.fixabilityScore.clarity +
    ...
    const compositeScore = riskScore * scoring.weights.issueComposite.risk +
 ...
 * Generate narrative text using templates
 */
function generateNarrativeText(template, cluster, claims, quotes, contradictionPairs, taxonomy) {
    ...
    const whyWrong = template.whyWrong.map((w) => renderTemplate(w, vars));
    const whyItMatters = template.whyItMatters.map((w) => renderTemplate(w, vars));
    const recommendedActions = template.recommendedActions.map((a) => ({
        type: a.type,
 ...
 * Get top edges for traceability
 */
function getTopEdgesForCluster(cluster, edges, limit) {
 ...
 */
function mapToSeverity(riskScore, scoring) {
    const thresholds = scoring.thresholds.severity;
    if (riskScore >= thresholds.critical * 100)
        return 'CRITICAL';
    if (riskScore >= thresholds.high * 100)
        return 'HIGH';
    if (riskScore >= thresholds.medium * 100)
        return 'MEDIUM';
    return 'LOW';
}
/**
 ...
 */
function mapToConfidence(compositeScore, scoring) {
    const thresholds = scoring.thresholds.confidence;
    if (compositeScore >= thresholds.high * 100)
        return 'HIGH';
    if (compositeScore >= thresholds.medium * 100)
        return 'MEDIUM';
    return 'LOW';
}
