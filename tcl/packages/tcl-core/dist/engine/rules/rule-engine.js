...
import { shouldConsiderContradiction, calculateTopicOverlap } from "../../claim_classifier.js";
/**
 * Run all rules to generate edges.
 */
export function runRuleEngine(claims, facts, config = DEFAULT_CONFIG) {
...
// ============================================================================
function findPolarityConflicts(subjectFacts, claims, config) {
    const edges = [];
    ...
    for (let i = 0; i < sorted.length; i++) {
        for (let j = i + 1; j < sorted.length; j++) {
            ...
            const isConflict = valuesConflict(earlier.normalizedValue, later.normalizedValue, earlier.valueType, later.valueType, earlier.polarity, later.polarity, config.normalization);
            if (isConflict) {
...
function findModalityShifts(claims, facts, config) {
    const edges = [];
...
function findAgentSelfContradictions(claims, facts, config) {
    const edges = [];
...
function findTimeframeConflicts(subjectFacts, claims, config) {
    const edges = [];
    const claimMap = new Map(claims.map(c => [c.id, c]));
    ...
    for (let i = 0; i < factsWithTime.length; i++) {
        ...
        for (let j = i + 1; j < factsWithTime.length; j++) {
            ...
            const overlap = timeframesOverlapNormalized(earlier.timeframeNormalized, later.timeframeNormalized, config.normalization.timeframeOverlapMap);
            if (overlap) {
...
function findSupportEdges(claims, facts, config) {
    const edges = [];
    const claimMap = new Map(claims.map(c => [c.id, c]));
    ...
    for (const [key, group] of factPairs) {
        ...
        for (let i = 0; i < sorted.length - 1; i++) {
            ...
            if (!srcClaim || !dstClaim)
                continue;
            const overlapScore = calculateTopicOverlap(srcClaim.text, dstClaim.text);
            if (overlapScore > 0.7 && !valuesConflict(earlier.normalizedValue, later.normalizedValue, earlier.valueType, later.valueType, earlier.polarity, later.polarity, config.normalization)) {
                // Paraphrase support
                let weight = config.edgeWeights.supportBase;
...
function findQuestionAnswerEdges(claims, config) {
    const edges = [];
...
function findRequestFulfillmentEdges(claims, config) {
    const edges = [];
...
// Helpers
// ============================================================================
function timeframesOverlap(a, b) {
    if (!a || !b)
        return false;
 ...
 */
function timeframesOverlapNormalized(a, b, overlapMap) {
    if (!a || !b)
        return false;
 ...
 */
function valuesConflict(valA, valB, typeA, typeB, polarityA, polarityB, normalization) {
...
function pruneEdges(edges, type, config) {
...
function generateEdgeId(prefix, srcId, dstId) {
