...
async function callSpectralService(spectralServiceUrl, claims, supports, contradictions, groundedClaimIds) {
    ...
    if (!res.ok)
        throw new Error(`Spectral service error: ${res.status}`);
...
async function validateOnce(input, adapter) {
    try {
        ...
        const scorer = options?.nliEndpoint
            ? new HttpNliScorer({
        ...
        if (spectralEnabled) {
            if (!spectralServiceUrl)
                throw new Error("spectral=true but no spectralServiceUrl/TCL_SPECTRAL_URL configured");
            spectral = await callSpectralService(spectralServiceUrl, evidenceRes.claims.map((c) => ({ id: c.id, text: c.text })), graph.supports, contradictions, graph.groundedClaimIds);
